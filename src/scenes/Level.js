...
export default class Level extends Phaser.Scene {
	...
	editorCreate() {
		...
		// obstacle
		const obstacle = new obstacleprefab(this, 2121, 540);
		...
		// ground
		const ground = new groundprefab(this, 640, 666);
		...
		jump_button.setDepth(100); // High depth value to ensure it's above obstacles
		uI.add(jump_button);
	...
	create() {
		...
		// Set jump button to appear in front of all game elements
		this.jump_button.setDepth(100); // High depth value to ensure it's above obstacles
		...
		if (this.isMobile) {
		 ...
		 // Make sure pause button is properly interactive
		this.pause_button.setInteractive({ useHandCursor: true });
	...
	setupMobileTouchControls() {
		...
		this.touchZone.on('pointerdown', () => {
		 ...
		 // Remove double-tap anywhere to pause functionality
	}
	...
	spawnNewObstacle() {
		...
		this.obstacle = new obstacleprefab(this, this.game.config.width + 100, 540, randomTexture);
		this.add.existing(this.obstacle);
		this.obstacle.setDepth(10); // Set obstacle depth lower than the jump button
